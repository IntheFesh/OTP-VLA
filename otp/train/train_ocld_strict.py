"""
otp/train/train_ocld_strict.py

Hydra-driven training loop for OCLD-Strict (OCLDStrictModel) — ablation baseline.

Config: configs/ocld_strict_frozen.yaml (default)

Usage:
  python -m otp.train.train_ocld_strict \\
...
logger = logging.getLogger("train_ocld_strict")


def _run(cfg) -> None:
    from otp.models.ocld_strict_model import OCLDStrictModel
    ...
    model = OCLDStrictModel(model_cfg).to(device)
    print_startup(cfg, model, device)

    if cfg.model.backbone_mode == "frozen":
    ...
    if synthetic:
        ...
        dataset = SyntheticDataset(
            ...
            num_objects=num_objects, horizon=horizon,
            num_grasps=num_grasps, num_points=num_points,
        )
        loader = DataLoader(
    ...
    if wandb_cfg and getattr(wandb_cfg, "enabled", False):
        try:
            import wandb
            wandb.init(project=wandb_cfg.project, config=model_cfg)
            use_wandb = True
        except ImportError:
            logger.warning("wandb not installed")

    out_dir = Path(getattr(cfg, "output_dir", "results/ocld_strict"))
    ...
    print()
    print(f"OCLD-Strict training complete in {elapsed_total:.1f}s  ({step} steps)")
...
def main() -> None:
    try:
        ...
        @hydra.main(
            config_path="../../configs",
            config_name="ocld_strict_frozen",
            version_base="1.2",
        )
        ...
        logger.error("hydra-core not installed. pip install hydra-core")
        sys.exit(1)
